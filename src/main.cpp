...
uint256 hashGenesisBlock("0x82269a9b95add2618d3b140b80a793c7a029cd695ee9fd5b6c7131e6da42115c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Patriotcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Patriotcoin Signed Message:\n";
...
{
    // Patriotcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Patriotcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Patriotcoin: 3 days
static const int64 nTargetSpacing = 5 * 60; // Patriotcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Patriotcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Patriotcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x312876bbcffe393c8d0e7c3a9c48b67113f8b70ee2bce6d35c67550cca75a27d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "3/9/2021 - Vape vaccine";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e55758a9ee92f7a175fbe5ba65912e75f1d8065053134ac5c596145421b7ef3b8603585a9992747bf1673f7e0bf7c5b6751e53f407fa44ede1e7658901fba224") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1615270005;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085631903;

        if (fTestNet)
        {
            block.nTime    = 1615269968;
            block.nNonce   = 386887039;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1e3b4a0f4298d50e058d5905568fbed53683caf751617186a29478cbc3251e24"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc4, 0xb9, 0xda }; // Patriotcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PatriotcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PatriotcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PatriotcoinMiner(CWallet *pwallet)
{
    printf("PatriotcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("patriotcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PatriotcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PatriotcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PatriotcoinMiner, pwallet));
}
